...
> Use the client only where you have permission. It provides no anti-cheat bypass, and all input-capable features require explicit authorization for every session.
...
The Config tab can save and load external-client preferences in `%LocalAppData%\Vectra External\client-config.json`. Saving is explicit; startup does not auto-load a profile. Input authorization is session-only and is always reset when loading a configuration.
